...
@auth_required
def create(user):
    d=request.get_json(silent=True) or {}; p=Project(name=(d.get("name") or "").strip(), tagline=d.get("tagline", ""), period=d.get("period", ""), description=d.get("description", ""), stack=d.get("stack", []), project_url=d.get("project_url", ""), video_url=d.get("video_url", ""))
...
def update(user, project_id):
    ...
    for key in ("name","tagline","period","description","stack","project_url","video_url"):
